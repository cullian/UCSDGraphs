...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		// list to hold degree sequence
		List<Integer> degreeSeq = new ArrayList<Integer>();
		// iterate thru each of the vertices
		for (int i = 0; i < getNumVertices(); i++) {
			// get neighbors
			List<Integer> outNeighbors = getNeighbors(i);
			List<Integer> inNeighbors = getInNeighbors(i);
			// count neighbors to get degree of vertice
			Integer degree = outNeighbors.size() + inNeighbors.size();
			// add degree to list for degree sequence
